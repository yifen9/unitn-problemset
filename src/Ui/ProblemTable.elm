...
import Html exposing (Html, a, div, node, span, table, tbody, td, text, th, thead, tr)
...
tdL x =
    td [ A.class "h-16 px-6 py-0 border-b-2 border-base-300/60 border-r-2 last:border-r-0" ]
        [ div [ A.class "h-full flex items-center text-2xl" ] [ x ] ]
...
view props =
    let
        nameCol =
            node "col" [ A.attribute "style" "width:75%" ] []

        idCol =
            node "col" [ A.attribute "style" "width:25%" ] []
    in
    table
